...
export class DayOne {
  ...
  partOne() {
    let exampleOne = '+1, -2, +3, +1';
    let exampleTwo = '+1, +1, +1';
    let exampleThree = '+1, +1, -2';
    let exampleFour = '-1, -2, -3';

    this.exampleOneAnswer = this.getAnswer(exampleOne);
    this.exampleTwoAnswer = this.getAnswer(exampleTwo);
    this.exampleThreeAnswer = this.getAnswer(exampleThree);
    this.exampleFourAnswer = this.getAnswer(exampleFour);
    let data = this.getAnswer(this.getMainData());

    this.mainAnswerAnswer = data.answer;
  ...
  partTwo() {
    ...
    this.partTwoExampleOneAnswer = this.getAnswerTwo(partTwoExampleOne);
    this.partTwoExampleTwoAnswer = this.getAnswerTwo(partTwoExampleTwo);
    this.partTwoExampleThreeAnswer = this.getAnswerTwo(partTwoExampleThree);
    this.partTwoExampleFourAnswer = this.getAnswerTwo(partTwoExampleFour);
  ...
  plotData(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      scatterData.x.push(i);
      scatterData.y.push(data[i]);
    }

    let element = document.getElementById('chart');
    Plotly.plot(element, [scatterData], {margin: { t: 0 }});
  }
  ...
  getAnswerTwo(input) {
    ...
    while (exit == false) {
      for (let i = 0; i < array.length; i++) {
        ...
        exit = answerArray.includes(answer);
